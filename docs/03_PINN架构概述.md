...
零偏置让初始网络保持相对对称。若需要指定初始温度水平，则由外层
`TemperatureField` 的 `theta_init` 逻辑设置各分支最后一层偏置。
...
`theta_init` 初始化时还会把最后一层偏置除以对应 `outscale`，确保所有区域乘回
不同输出尺度后仍得到相同的初始无量纲温度。若改变 `_OUTPUT_SCALE`，应重新
训练或重新初始化，不能把它当作只影响显示结果的后处理系数。
...
指定 `theta_init` 时，代码把各分支最后一层权重设为零，并将偏置设置为：
...
$L=2\pi R_3$，方块单边使用 $D_1$ 或 $D_2$。这相当于在界面上进行均匀随机
蒙特卡洛积分。
...
普通独立随机采样的统计误差通常按 $O(N^{-1/2})$ 下降，所以增加点数会使积分
结果更稳定，但收益不是线性的：误差约减半通常需要约四倍点数。增加采样点只能
降低积分抽样误差，不能修正温度网络、自动微分梯度、导热系数、法向或界面长度
本身的误差。角点和热流陡变区域还可能需要局部加密。

当前规则边界也适合使用等距中点、Gauss--Legendre 或低差异序列等确定性/准
蒙特卡洛求积。热流足够光滑时，它们可能以更少的点获得比纯随机采样更稳定的
能量预算；随机采样的优势则是实现简单，并能在训练期间不断覆盖不同位置。
...
`total` 保留计算图并参与反向传播；`details` 中的带符号相对残差经过
`detach()`，只用于日志和诊断：正值表示排热超过目标，负值表示排热不足。
...
优先选择 `cuda:0`，没有 CUDA 时退回 CPU。网络采用统一的 `theta_init` 温度
初值，并可按域使用 Fourier 特征。
...
| `--resume --resume-lr <值>` | 完整恢复后覆盖当前 Adam 学习率，同时保留动量和调度进度 |
...
同时保存按训练步编号的 `epoch_XXXXXX.pt`，并刷新便于续训的 `latest.pt`。
L-BFGS 历史快照额外包含拟牛顿优化器状态。训练结束后强制恢复目标全功率，
...
| 参数组 | 默认设置 |
|---|---|
| 网络 | `width=96`，`depth=5`，`theta_init=0` |
...
| 损失权重 | PDE 1，设备 PDE 100，界面温度/通量各10，边界20，能量30 |
...
`theta_init` 是所有分域的统一初始无量纲温度。默认0对应冷端参考温度。已有相近
工况时优先用 checkpoint 热启动；手工提高 `theta_init` 只能改变初始常温水平，
不能提供正确的温度梯度和界面热流。
...
4. 设备安装面及所有材料界面的温度、热流连续误差合格；
5. 三个设备最高温度在加密监控网格上稳定；
6. `validate.py` 的逐域及全局 FEM 相对误差满足复合精度要求；
