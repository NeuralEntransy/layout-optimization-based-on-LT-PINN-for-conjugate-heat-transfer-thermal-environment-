...
for ax, name in zip(axes, layouts):
    d = np.loadtxt(os.path.join(outdir, f"Tfield_{name}.csv"),
...
fig.suptitle("Milestone 0 FEM reference — stagnant-air conduction\n"
             "(color card clipped to [-55, 150] °C; cyan contour = 70 °C limit)")
fig.savefig(os.path.join(outdir, "Tfield_all_layouts.png"), dpi=150)
print("saved", os.path.join(outdir, "Tfield_all_layouts.png"))
